    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-muted-foreground font-medium animate-pulse">Loading secure invoice...</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-6">
        <Card className="bg-white/5 border-white/10 max-w-md w-full p-8 text-center rounded-[2.5rem]">
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12 print:hidden">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">Verified by Daniele Buatti IT</p>
            </div>
          </div>
          
          <div className="flex gap-3">
            <Button variant="outline" className="rounded-xl border-slate-200 bg-white text-slate-900 hover:bg-slate-50" onClick={() => window.print()}>
          ...
          <CardContent className="p-8 md:p-16 space-y-12">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start gap-8">
              <div>
                ...
                <div className="text-xs text-slate-400 mt-6 space-y-2">
                  {settings?.company_abn && <div className="flex items-center gap-2"><Badge variant="outline" className="text-[9px] py-0">ABN</Badge> {settings.company_abn}</div>}
                  {settings?.company_email && <div className="flex items-center gap-2"><Mail className="h-3 w-3" /> {settings.company_email}</div>}
                  {settings?.company_phone && <div className="flex items-center gap-2"><Phone className="h-3 w-3" /> {settings.company_phone}</div>}
                </div>
              </div>
              
              <div className="text-left md:text-right">
                <div className="text-4xl md:text-5xl font-black text-slate-100 mb-6">
                  {isTaxInvoice ? 'TAX INVOICE' : 'INVOICE'}
                </div>
                <div className="space-y-2">
                  <div className="text-lg font-bold text-slate-900">{invoice.number}</div>
                  <div className="text-sm text-slate-500">Issued: {format(new Date(invoice.invoice_date), 'MMMM d, yyyy')}</div>
            ...
            <div className="grid md:grid-cols-2 gap-12 pt-12 border-t border-slate-100">
              <div>
                ...
                <div className="text-sm text-slate-500 mt-2">Client Reference: {invoice.client_id?.slice(0, 8)}</div>
              </div>
              
              <div className="flex flex-col md:items-end">
                <div className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-4">Status</div>
              ...
              <table className="w-full min-w-[600px]">
                <thead>
                  <tr className="border-b-2 border-slate-900 text-left">
                    <th className="pb-4 text-[10px] font-bold uppercase tracking-widest text-slate-400">Description</th>
                    <th className="pb-4 text-[10px] font-bold uppercase tracking-widest text-slate-400 text-center">Qty</th>
                    <th className="pb-4 text-[10px] font-bold uppercase tracking-widest text-slate-400 text-right">Rate</th>
                    <th className="pb-4 text-[10px] font-bold uppercase tracking-widest text-slate-400 text-right">Amount</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {invoice.line_items?.map((item: any, i: number) => (
                    <tr key={i}>
                      <td className="py-8 font-medium text-slate-900">{item.description}</td>
            ...
            <div className="flex justify-end pt-12">
              <div className="w-full md:w-72 space-y-4 bg-slate-50 p-8 rounded-3xl">
                <div className="flex justify-between text-sm">
                  <span className="text-slate-500">Subtotal</span>
                  <span className="font-medium text-slate-900">${invoice.untaxed_amount?.toFixed(2)}</span>
                </div>
                {isTaxInvoice && (
                  <div className="flex justify-between text-sm">
            ...
            <div className="pt-12 border-t border-slate-100">
              <div className="grid md:grid-cols-2 gap-12">
                <div>
                  <div className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-6">Payment Instructions</div>
                  <div className="space-y-4">
                    <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100">
                      <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Bank Transfer (EFT)</p>
                ...
                <div className="flex flex-col justify-end">
                  <div className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-4">Terms</div>
            ...
            <div className="pt-24 text-[10px] text-slate-300 uppercase tracking-[0.3em] text-center font-bold">
        ...
        <div className="mt-12 text-center print:hidden">
          <p className="text-sm text-slate-400">
            This is a secure digital invoice. For any billing enquiries, please contact <a href={`mailto:${settings?.company_email}`} className="text-primary hover:underline">{settings?.company_email}</a>
          </p>
        </div>
      </div>
    </div>
